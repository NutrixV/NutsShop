...
2. **Вирішення - перевірка і генерація ключа при запуску**:
   - Додайте в скрипт `docker-entrypoint.sh` код для перевірки та генерації ключа:
     ```bash
     ...
     ```
   - Цей код забезпечує, що:
     1. Якщо ключ відсутній або має неправильний формат, буде створено новий
     2. Новий ключ буде доступний як змінна середовища для всіх процесів
     3. Вміст .env файлу буде перезаписано новим ключем

3. **Збереження ключа в Render.com**:
   - Після першого успішного запуску контейнера скопіюйте згенерований ключ з логів
   - Додайте його як статичне значення в `render.yaml`:
     ```yaml
     - key: APP_KEY
       value: "base64:xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx="  # Фактичний ключ з логів
     ```
   - Або додайте його через інтерфейс Render.com у змінні середовища сервісу
   - Це забезпечить постійне використання одного й того ж ключа шифрування при всіх перезапусках
