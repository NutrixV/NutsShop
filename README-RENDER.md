...
3. **Вирішення - перевірка існування таблиць**:
   - У скрипті `docker-entrypoint.sh` перевіряємо наявність таблиць перед створенням міграцій:
     ```bash
     # Перевіряємо, чи існують таблиці в базі даних
     ...
     ```
   - Це запобігає повторному створенню міграцій, якщо таблиці вже існують

4. **Процес обробки черг**:
