...
- Запускається в Docker-контейнері
- Використовує Node.js 16 (стабільна версія для Nuxt)
- Використовує двоетапну збірку для оптимізації розміру образу
- Пропускає перевірку TypeScript для уникнення помилок збірки
...
Якщо виникають проблеми зі збіркою фронтенду через помилки TypeScript:

1. Файл `frontend/Dockerfile.alt` використовує двоетапну збірку та Node.js 16, що допомагає уникнути проблем із сумісністю
2. У файлі `render.yaml` встановлено змінну оточення `NUXT_SKIP_TS_CHECK=true`
3. В команді збірки `package.json` встановлено `--skipTypeCheck`
...
2. У Dockerfile бекенду вже додано цей пакет до списку залежностей
3. Для інших розширень PHP можливо також знадобиться встановити додаткові dev-пакети 